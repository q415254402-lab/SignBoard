...
| 📡 ADB 远程配置 | 通过 ADB 广播修改服务器地址和设备名称，支持无屏设备自动部署 |
...
# 2. 授权系统设置权限
python tools/adb_config.py --grant

# 3. 配置服务器地址和设备名称（自动注册+启动）
python tools/adb_config.py --url http://192.168.3.254:8000 --name "一楼大厅"

# 或使用 GUI 工具
python tools/adb_config_gui.py
```

GUI 工具支持：自动检测设备、无线 ADB 连接、一键配置、权限授权。
...
**ADB 远程配置（新增）：**
- ConfigReceiver 广播接收器：通过 ADB 修改服务器地址和设备名称
- 自动注册服务器 + 自动启动播放器（无屏设备无需触屏操作）
- GUI 配置工具（`tools/adb_config_gui.py`）：自动检测设备、无线连接、一键配置
- 命令行配置工具（`tools/adb_config.py`）：支持 `--url`/`--name`/`--grant`/`--clear`
- ADB 授权系统设置权限（`WRITE_SETTINGS` + `SYSTEM_ALERT_WINDOW`）
